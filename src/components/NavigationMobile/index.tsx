...
const NavigationMobile = ({ setIsOpenMenu }: NavigationMobileProps) => {
  return (
    <div className="dark:bg-scale-300 fixed -inset-y-0 z-50 h-screen w-screen transform overflow-y-scroll bg-white p-4 md:p-8 !bg-scale-300 enter-done">
      ...
      <div className="mt-6 mb-12">
        ...
        {categoriesNav.map((item, index) => (
          <div className="pt-2 pb-4 space-y-1" key={index}>
            <Link
              className="block py-2 pl-3 pr-4 text-md font-medium rounded-md text-scale-250 dark:hover:bg-scale-600 hover:border-gray-300 hover:bg-gray-50 dark:text-white"
              href={item.path}
            >
              {item.title}
            </Link>
